...
def load_radio_map():
    ...
    for line in fp.readlines():
        ...
        # 하나하나의 숫자는 134개의 ap 별 rss 신호를 의미
...
def load_ap_list_importance():
    ap_list = {}
    
    fp = open('ap_list_short.txt', 'r')
...
def calculate_client_position(radio_map, client_rss_store_median):
    ap_num = 107; row = 18; col = 16
    
    min_dist = 9999999999 # 매우 큰 값으로 초기화
    result_x = 0; result_y = 0; diff = 0
    ...
    for x in range(row):
        for y in range(col):
            ...
            for ap_idx in range(ap_num):
                if(client_rss_store_median[ap_idx] != 0): # 실시간으로 측정된 ap에 대해서만 계산 진행
                    diff = client_rss_store_median[ap_idx] - radio_map[x][y][ap_idx]
                    diff = pow(diff, 2)
